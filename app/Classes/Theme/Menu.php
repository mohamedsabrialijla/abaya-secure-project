...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        if (isset($item['separator'])) {
            ...
            if (isset($item['parent'])) {
                ...
                echo '<a ' . $target . ' href="' . $url . '" class="menu-link normalLink ' . (isset($item['submenu']) ? 'menu-toggle' : '') . '">';
            }
            ...
            foreach ($item as $each) {
                ...
                if(isset($each['permission_name'])){
                    $action='view';
                    if(isset($each['module_action'])){
                        $action=$each['module_action'];
                    }

                   if(\Auth::guard("system_admin")->user()->canany($each['permission_name'],'system_admin')){
    ...
    public static function isActiveVerMenuItem($item, $page, $rec = 0)
    {
        ...
        if(isset($item['search_pattern'])){
            if(Request::is('*/'.$item['search_pattern'])){
                return true;
            }
        }
        if(isset($item['active_subs'])){
            if(is_array($item['active_subs'])){
                foreach ($item['active_subs'] as $ii){
                    $currentRoute=\Illuminate\Support\Facades\Route::current()->getPrefix();
                    $name=route($ii);
                    if(url($currentRoute) == $name){
                        return true;
