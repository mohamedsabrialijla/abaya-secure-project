...
@section('page_content')
    ...
    @component('components.AddEditCard', [
        ...
        <div class="row">
            ...
            {{-- <div class="col-md-6">
                @component('components.selectWithAdd', ['data' => $out->store_id, 'name' => 'store_id', 'text' => 'المصمم',
                    'select' => $stores, 'add_url' => route('system.stores.createJson')])
                @endcomponent
            </div> --}}

            <div class="col-md-6">
                @component('components.input', ['data' => $out->code, 'name' => 'code', 'text' => 'الكود', 'placeholder' =>
                    'ادخل الكود'])
                @endcomponent
            </div>

            <div class="col-md-6">
                @component('components.switch', ['data' => $out->is_active ?? '', 'name' => 'is_active', 'text' => 'حالة
                    الكود'])
                @endcomponent
            </div>
            ...
            <div class="w-100"></div>
            <div class="col-md-6">
                @component('components.input', ['data' => $out->discount_ratio, 'name' => 'discount_ratio', 'text' => 'نسبة او قيمة
            ...
            <div class="col-md-6 d-flex flex-stack mb-8">
                ...
                <label class="form-check form-switch form-check-custom form-check-solid">
                    <input class="form-check-input" name="flag" type="radio" value="1" @if ($out->flag == 1) checked="checked"
                    @endif >
                    <div class="flipthis-wrapper"><span class="form-check-label fw-bold text-muted">نسبة مئوية</span></div>
                    <hr>
                    <input class="form-check-input" name="flag" type="radio" value="2" @if ($out->flag == 2) checked="checked"
                    @endif>
                    <div class="flipthis-wrapper"><span class="form-check-label fw-bold text-muted">مبلغ محدد</span></div>
                    <hr>
                    <input class="form-check-input" name="flag" type="radio" value="3" @if ($out->flag == 3) checked="checked"
                    @endif>
            ...
            <div class="w-100"></div>
            <div class="col-md-6">
                @component('components.input', ['data' => $out->limit,'name' => 'limit', 'text' => 'الحد الادني لقيمة الطلب لتفعيل الكوبون', 'icon' => 'fa-dollar'])
                @endcomponent
            </div>
            <div class="col-md-6">
                @component('components.switch', ['data' => $out->show ?? '','name' => 'show', 'text' => 'ظهور الكوبون تحت المنتج '])
                @endcomponent
            </div>
            ...
            <div class="col-md-6">
                @component('components.input', ['data' => $out->count_of_use, 'name' => 'count_of_use', 'text' => 'عدد مرات
                    الاستخدام'])
                @endcomponent
            </div>
            ...
            <div class="w-100"></div>
            <div class="col-md-6">
                @component('components.datepicker', ['data' => \Carbon\Carbon::parse($out->start_date)->format('m/d/Y'), 'name'
                    => 'start_date', 'text' => 'تاريخ البداية'])
                @endcomponent
            </div>

            <div class="col-md-6">
                @component('components.datepicker', ['data' => \Carbon\Carbon::parse($out->expire_date)->format('m/d/Y'), 'name'
