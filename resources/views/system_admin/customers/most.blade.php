            ...
            <div class="col">
                <form class="form-inline" id="form" style="float: right">
                    <div class="form-group m-form__group">
                        @component('components.serach.selectArr', ['key' => 'status', 'text' => 'اختر الحالة', 'select' => [0 => 'غير مفعل', 1 => 'مفعل']])
                        @endcomponent
                        ...
                        @foreach ($out as $o)
                            @if ($o)
                                <tr id="TR_{{ $o->customer->id }}">
                                    ...
                                    <td style="text-align: center;vertical-align: middle;">
                                        <label class="checkbox checkbox-outline checkbox-success justify-content-center">
                                            <input type="checkbox" value="<?= $o->customer->id ?>" name="Item[]"
                                                class="CheckedItem" id="che_{{ $o->customer->id }}">
                                            <span></span>
                                        </label>
                                    </td>
                                    <td class="text-center">{{ $o->customer->name }}</td>
                                    <td class="text-center"> {{ $o->customer->email }}</td>
                                    <td class="text-center"> {{ $o->customer->mobile }}</td>

                                    {{-- <td class="text-center">
                                    @if (auth('system_admin')->user()->can('activate_users', 'system_admin'))
                                        <div class="form-group" id="status_{{ $o->customer->id }}">
                                            <div class="radio-inline">
                                                <label class="radio">
                                                    <input type="radio" class="change_status" name="status_{{ $o->customer->id }}"
                                                        data-id="{{ $o->customer->id }}" data-status="1" value="1"
                                                        {{ $o->customer->status == 1 ? 'checked' : '' }} />
                                                    <span></span>
                                                    مفعل
                                                </label>
                                                <label class="radio">
                                                    <input type="radio" class="change_status"
                                                        name="status_{{ $o->customer->id }}" data-id="{{ $o->customer->id }}"
                                                        data-status="0" value="0" {{ $o->customer->status == 0 ? 'checked' : '' }} />
                                                    <span></span>
                                                    غير مفعل
                                                </label>
                                    ...
                                    @else
                                        @if ($o->customer->status == 1)
                                            <span class="font-success"> مفعل </span>
                                        @elseif($o->customer->status == 2)
                                            <span class="font-danger"> محظور </span>
                                        @elseif($o->customer->status == 0)
                                            <span class="font-warning"> غير مفعل </span>
                                        @endif
                                    @endif
                                   </td> --}}

                                    <td class="text-center"> {{ $o->customer->created_at->toDateString() }}</td>
                                    <td class="text-center"> {{ $o->purchase_total }}</td>
                                    <td class="text-center"> {{ $o->customer->orders->count() }}</td>
                                    <td class="text-center">

                                        <ul class="list-inline">
                                            @if (auth('system_admin')->user()->can('view_users', 'system_admin'))
                                                <li>
                                                    <a href="{{ route('system.users.details', $o->customer->id) }}"
