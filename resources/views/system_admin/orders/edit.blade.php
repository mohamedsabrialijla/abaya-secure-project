                    ...
                    <div class="card-toolbar">
                        <div class="card-toolbar">
                            ...
                            @if (auth('system_admin')->user()->can('edit_orders', 'system_admin'))
                                ...
                                <button type="button" class="{{ config('layout.classes.edit') }} mt-2" data-toggle="modal"
                                ...
                                <div class="modal fade" tabindex="-1" id="editpay">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{ route('system.orders.editpay', ['id' => $order->id]) }}"
                                                method="post">
                                                @csrf
                                                    ...
                                                    <div class="col-md-6 d-flex flex-column mb-8 fv-row">
                                                        ...
                                                        <select class="form-select form-select-solid" data-control="select2"
                                                            ...
                                                            @foreach ($paymentTypes as $pay)
                                                                <option value="{{ $pay->id }}"
                                                                    @if ($order->payment_type_id == $pay->id) selected @endif>
                                                                    {{ $pay->name_ar }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                ...
                                <div class="col-sm-9">
                                    <div class="order-status-btn">
                                        @if (auth('system_admin')->user()->can('edit_orders', 'system_admin'))
                                            ...
                                            @if($order->case_id == 4 || $order->case_id == 5|| $order->case_id == 6|| $order->case_id == 7|| $order->case_id == 8)

                                            <a href="http://www.sls-express.com/api/v1/order/waybill?api_token=IhexECgpAL84uzUb8uYWzdT1iDktRaX7f7nXjSE28t&tracking_number={{ $order->shipment_id }}" target="_blank" class="{{ config('layout.classes.add') }}">
                                            طباعة البوليصة
                                            </a>
                                            <a href="http://www.sls-express.com/tracking?tracking_number={{ $order->shipment_id }}" target="_blank"  class="{{ config('layout.classes.add') }}">
                                                تتبع الشحنة
                                            </a>
                                        ...
                                        @endif
                                        {{-- @if (auth('system_admin')->user()->can('edit_orders', 'system_admin'))
                                            <form action="{{ route('system.orders.change_status') }}" method="POST">
                                                @csrf
                                                <input type="hidden" name="id" value="{{ $order->id }}">
                                                @if ($order->case_id == 1)
                                                    <button class="{{ config('layout.classes.add') }} ">
                                                        تأكيد
                                                    </button>
                                                @elseif($order->case_id == 3)
                                                    <button class="{{ config('layout.classes.add') }} ">
                ...
                <div class="card-header">
                    ...
                    <div class="card-toolbar">
                        <div class="card-toolbar">
                            @if (auth('system_admin')->user()->can('edit_orders', 'system_admin'))
                                <!-- Button trigger modal -->
                                <button type="button" class="{{ config('layout.classes.edit') }} mt-2"
                                ...
                                <div class="modal fade" tabindex="-1" id="addproduct">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{ route('system.orders.addproduct', ['id' => $order->id]) }}"
                                                method="post">
                                                @csrf
                                                <div class="modal-body">
                                                    <div class="col-md-6 d-flex flex-column mb-8 fv-row">
                                                    ...
                                                    <div class="col-md-6 d-flex flex-column mb-8 fv-row">
                                                        ...
                                                        <select class="form-select form-select-solid"
                                                            ...
                                                            @foreach ($sizes as $size)
                                                                <option value="{{ $size->id }}">{{ $size->name_ar }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    ...
                                                    <div class="col-md-6">
                                                        @component('components.input', ['name' => 'qty', 'text' => 'الكمية'])
                                                        @endcomponent
                                                    </div>

                                                    <div class="col-md-6">
                                                        @component('components.input', ['name' => 'replace', 'text' => 'رسوم الاستبدال', 'icon' => 'fa-dollar'])
                                                        @endcomponent
                                                    </div>
                                                </div>
                                        ...
                                        <td>
                                            @if (auth('system_admin')->user()->can('edit_orders', 'system_admin'))
                                                <button class="{{ config('layout.classes.delete') }} mt-2 btn-del"
                                                    ...
                                                    data-url="{{ route('system.orders.deleteproduct') }}"
